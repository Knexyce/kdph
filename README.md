# KGPH: Knexyce GitHub Package Handler

**KGPH** is a lightweight package manager for managing software simply. It allows developers to package their code securely, distribute it, and retrieve it with minimal setup.
...
* Code is archived and encrypted into a Knexyce Package.
...
KGPH is flexible, but here are example layouts to help you get started.
...
│   ├── libcfg.py           # Uses KGPH's 'getpkg' to install code into the 'lib' folder.
│   └── placeholder.json    # Example configuration file.
├── core/                   # Central package logic; may use KGPH or pip-installed code.
│   ├── __init__.py         # Enables imports from this subpackage.
...
└── utils/                  # Utility code.
    ├── __init__.py         # Enables imports from this subpackage.
    ├── kgph.py             # Local copy of KGPH, used by 'libcfg.py'.
...
├── __main__.py            # Installs KGPH packages into 'lib' and compiles the central project.
├── include/               # Header files.
│   └── placeholder.h      # Example header, compiled via CMake.
├── kgph.py                # Functions for installing C binaries or sources into 'lib'.
...
```bash
git clone https://github.com/Knexyce/kgph.git
cd kgph
...
KGPH supports three main commands.
...
```bash
python3 kgph.py mkpkg -f <folder> -k <encryption_key> -t <github_pat>
```
...
```bash
python3 kgph.py rmpkg -p <package_name> -t <github_pat>
```
